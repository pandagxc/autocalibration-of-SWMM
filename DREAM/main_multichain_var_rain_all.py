...
import mymodel_varperv_var_rain_all
...
sampler = multichain_mcmc.DreamSampler(mymodel_varperv_var_rain_all.model_gen)  # 11 params
#sampler.sample(nChains=10, ndraw=3000)
sampler.sample(nChains=30, ndraw=10000, ndraw_max=1000000)
print(sampler.R)
history = sampler.history
#history = history[-3000:, :]
os.chdir(path_parms)
np.savetxt('params_var_rain_all_uniform1.5_10p.txt', history, fmt='%f', delimiter=',')
#np.savetxt('params_var_rain_all_uniform_8p.txt', history, fmt='%f', delimiter=',')
