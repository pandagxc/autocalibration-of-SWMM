...
typedef struct
{
   ...
   double        S;               // suction pressure (ft)
   double        Ks;              // Sat. Hyd. conductivity (ft/sec)
   double        IMDmax;          // Max. moisture deficit
   double        rainmultiplier;  // rain multiplier change factor, used to modify rainfall
   //...                          // intensity for input uncertatinty analysis. Xichao Gao.
   double        pctzero;         // percent of non storage impervious areas to impervious areas (100). Xichao Gao
   double        pctrouted;       // percent of water routing from impervious areas to pervious areas or vice versa. Xichao Gao
   double        routeto;         // where the subarea water route to. Xichao Gao
}  TSubcatch;
...
typedef struct
{
   ...
   // add manning's roughness coefficient. Xichao Gao
