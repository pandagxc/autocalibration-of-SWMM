...
//=============================================================================
int subcatch_readInfiParams(int m, char* tok[], int ntoks)
///
//  Written by Xichao Gao 2/16/2020
//  Used to write infiltration parameters to subcatch object.
//  Warning: Only green-ampt infiltration method was modified. Other infiltration method can no longer be used.
//           m  infiltration method
//           tok[] = array of string tokens
//           ntoks = number of tokens
//  Output:  returns an error code
//  Purpose: sets infiltration parameters from a line of input data to subcatch object.
//
...
//     subcatch  p1  p2 ...
{
    ...
    return FALSE;
    //infil->S      = x[0] / UCF(RAINDEPTH);    // Capillary suction head (ft)
    //infil->Ks     = x[1] / UCF(RAINFALL);     // Sat. hyd. conductivity (ft/sec)
    //infil->IMDmax = x[2];                     // Max. init. moisture deficit


    // --- assign input values to subcatch's properties
    Subcatch[j].S            = x[0] / UCF(RAINDEPTH);   // Capillary suction head (ft)
    Subcatch[j].Ks           = x[1] / UCF(RAINFALL);    // Sat. hyd. conductivity (ft/sec)
    Subcatch[j].IMDmax       = x[2];                    // Max. init. moisture deficit

    //printf("subcatch %d infiltration rate\n", j);
    //printf("%f\n", Subcatch[j].Ks);
    //printf("%f\n", x[1]);
    //printf("%f\n", x[1]/UCF(RAINFALL));
...
//
{
    ...
    Subcatch[j].rainmultiplier = 1;      // set initial value to rainfall multiplier. Xichao Gao
...
//
{
    ...
    Subcatch[j].pctrouted = x[6]; // shared by Subcatch and Subarea. added by Xichao Gao
    Subcatch[j].routeto   = x[5]; // shared by Subcatch and Subarea. added by Xichao Gao
    ...
    Subcatch[j].pctzero                 = x[4] / 100.0; //shared by Subcatch and Subarea. added by Xichao Gao.
    ...
    Subcatch[j].subArea[IMPERV0].fArea  = Subcatch[j].fracImperv * Subcatch[j].pctzero;
    Subcatch[j].subArea[IMPERV1].fArea  = Subcatch[j].fracImperv * (1.0 - Subcatch[j].pctzero);
    ...
    if ( k == TO_IMPERV && Subcatch[j].fracImperv )
    {
        ...
        Subcatch[j].subArea[PERV].fOutlet = 1.0 - Subcatch[j].pctrouted;
    }
    ...
    if ( k == TO_PERV )
    {
        ...
        Subcatch[j].subArea[IMPERV0].fOutlet = 1.0 - Subcatch[j].pctrouted;
        Subcatch[j].subArea[IMPERV1].fOutlet = 1.0 - Subcatch[j].pctrouted;
    }
    return 0;
}

int subcatch_setSubareaParams(int j)
//
//  added by Xichao Gao.
//  Input:   subcatchment index
//  Output:  returns an error code
//  Purpose: used to set subareaparams that affected by fracImperv through python api.
//
//  Data has format:
//    Subcatch  Imperv_N  Perv_N  Imperv_S  Perv_S  PctZero  RouteTo (PctRouted)
//
{
    ...
    // --- assign input values to each type of subarea
    //Subcatch[j].subArea[IMPERV0].N = Subcatch[j].subArea[IMPERV0].N;
    //Subcatch[j].subArea[IMPERV1].N = Subcatch[j].subArea[IMPERV1].N;
    //Subcatch[j].subArea[PERV].N    = Subcatch[j].subArea[PERV].N;
    ...
    // Subcatch[j].subArea[IMPERV0].fArea  = Subcatch[j].fracImperv * x[4] /
    // 100.0; Subcatch[j].subArea[IMPERV1].fArea  = Subcatch[j].fracImperv * (1.0
    // - x[4] / 100.0);
    Subcatch[j].subArea[IMPERV0].fArea =
        Subcatch[j].fracImperv * Subcatch[j].pctzero;
    Subcatch[j].subArea[IMPERV1].fArea =
        Subcatch[j].fracImperv * (1.0 - Subcatch[j].pctzero);
    Subcatch[j].subArea[PERV].fArea = (1.0 - Subcatch[j].fracImperv);

    // --- assume that all runoff from each subarea goes to subcatch outlet
    // the initial routeto value has been assigned in the subcatch_readparams function. Xichao Gao.
    //for (i = IMPERV0; i <= PERV; i++) {
    //    Subcatch[j].subArea[i].routeTo = TO_OUTLET;
    //    Subcatch[j].subArea[i].fOutlet = 1.0;
    //}
    ...
    // k = (int)x[5];
    k = (int)Subcatch[j].routeto;
    if (Subcatch[j].fracImperv == 0.0 || Subcatch[j].fracImperv == 1.0)
        k = TO_OUTLET;
    if (k == TO_IMPERV && Subcatch[j].fracImperv) {
        Subcatch[j].subArea[PERV].routeTo = k;
        Subcatch[j].subArea[PERV].fOutlet = 1.0 - Subcatch[j].pctrouted;
...
//// isUsed property of subcatchment's rain gage now set in subcatch_validate  //(5.1.013)
    ...
    //Subcatch[j].Ks = 0.0;
    //Subcatch[j].S = 0.0;
    //Subcatch[j].IMDmax = 0.0;
    //Subcatch[j].rainmultiplier = 0.0;
    

    // --- initialize state of infiltration, groundwater, & snow pack objects
    if ( Subcatch[j].infil == j )  infil_initState_sub(j, InfilModel);
...
//
{
    ...
    // add rainmultiplier to change rainfall intensity. Xichao Gao
    //vRain = Subcatch[j].rainmultiplier * Subcatch[j].rainfall * tStep * area;
...
//
    ...
    // add rainmultiplier for considering uncertainty of precipitation. Xichao Gao
    Subcatch[j].rainfall = (rainfall + snowfall) * Subcatch[j].rainmultiplier;

    //printf("rainmultiplier in routing runoff %f", Subcatch[j].rainmultiplier);
    ...
    else
    {
        ...
        // modified by Xichao Gao.
        for (i=IMPERV0; i<=PERV; i++) netPrecip[i] = (rainfall + snowfall) * Subcatch[j].rainmultiplier;
    }
}
