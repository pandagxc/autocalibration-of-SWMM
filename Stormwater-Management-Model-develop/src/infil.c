...
//     subcatch  p1  p2 ...
{
    ...
    switch (m)
    {
      ...
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
                         //从subcatch里面把参数读给GAInfil
                         status = grnampt_setParams_subcatch(&GAInfil[j], x, j);
                         break;
...
void infil_initState_sub(int j, int m)
//
//  written by Xichao Gao.
//  Input:   j = subcatchment index
//           m = infiltration method code
//  Output:  none
//  Purpose: initializes state of infiltration for a subcatchment.
//
{
    switch (m) {
        case HORTON:
        case MOD_HORTON:
            horton_initState(&HortInfil[j]);
            break;
        case GREEN_AMPT:
        case MOD_GREEN_AMPT:
            grnampt_initState_sub(&GAInfil[j], j);
            break;
        case CURVE_NUMBER:
            curvenum_initState(&CNInfil[j]);
            break;
    }
}
...
int grnampt_setParams_subcatch(TGrnAmpt *infil, double p[], int j)
//
//  read infiltration parameters to infiltration object
//  Written by Xichao Gao. 02/16/2020
...
//
{
    ...
    infil->S = Subcatch[j].S;
    infil->Ks = Subcatch[j].Ks;
    infil->IMDmax = Subcatch[j].IMDmax;

    //printf("subcatchment %d infiltration\n", j);
    //printf("ks in setParams subcatchment %f\n", infil->Ks);
...
//
{
    ...
    infil->Sat = FALSE;
    infil->T = 0.0;
    //used to change infiltration params after function readinput(). added by Xichao Gao.
    //infil->Ks  = Subcatch[j].Ks;
    //infil->S   = Subcatch[j].S;
    //infil->IMD = Subcatch[j].IMDmax;
    //printf("grnampt_initstate IMDmax %f\n", infil->IMDmax);
}

void grnampt_initState_sub(TGrnAmpt *infil, int j)
//
//  Input:   infil = ptr. to Green-Ampt infiltration object
//  Output:  none
//  Purpose: initializes state of Green-Ampt infiltration for a subcatchment.
//
{
    ...
    infil->Sat = FALSE;
    infil->T   = 0.0;
    // used to change infiltration params after function readinput(). added by
    // Xichao Gao.
    infil->Ks  = Subcatch[j].Ks;
    infil->S   = Subcatch[j].S;
    infil->IMD = Subcatch[j].IMDmax;
    //printf("grnampt_initstate IMDmax %f\n", infil->IMD);
    //printf("grnampt_initstate ks %f\n", infil->Ks);
    //printf("grnampt_initstate suction %f\n", infil->S);
}
...
//
{
    ...
    //printf("ks in getinfil (previous step of sat/unsat infiltration %f\n",
    //      infil->Ks);
...
//
{
    ...
    //printf("Ks in unsat infiltration %f\n", ks);
...
//
{
    ...
    //printf("Ks in sat infiltration function %f\n", ks);
