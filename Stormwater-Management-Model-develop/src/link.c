...
//
{
    Link[j].roughness = 0; // set initial value to roughness of link object. Xichao Gao
    switch ( type )
    {
...
//
{
    ...
    Link[j].direction   = 1;
    // add roughness properties to Link object. Xichao Gao
    Link[j].roughness = x[1];
    ...
    switch (type)
    {
      case CONDUIT:
        ...
        Conduit[k].roughness = Link[k].roughness; // read roughness from Link object. Xichao Gao
...
//
{
    ...
    if ( !getDouble(tok[4], &x[1]) ) // x[1] represents roughness. Xichao Gao
...
//
{
    ...
    Conduit[k].roughness = Link[k].roughness;
    Link[j].newDepth = link_getYnorm(j, Link[j].q0 / Conduit[k].barrels);
    Link[j].oldDepth = Link[j].newDepth;
    //printf("Conduit %d roughness is %f\n", k, Conduit[k].roughness);
}
