    ...
    *value = 0;
    ...
    else
    {
        switch(Param)
        {
            ...
            case SM_SEEPRATE:
                *value = Link[index].seepRate * UCF(RAINFALL); // added by Xichao Gao
                break;
            case SM_CONROUGHNESS:  // add link manning's roughness param. Xichao Gao
                *value = Link[index].roughness;
                break;
            default: error_code_index = ERR_API_OUTBOUNDS; break;
        }
    }
    ...
    else
    {
        switch(Param)
        {
            ...
            case SM_SEEPRATE:
                ...
                break;
            case SM_CONROUGHNESS:  // add link roughness params. Xichao Gao.
                Link[index].roughness = value;
                break;
            default: error_code_index = ERR_API_OUTBOUNDS; break;
        }
        // re-validated link
    ...
    *value = 0;
    ...
    else
    {
        switch(Param)
        {
            ...
            case SM_SLOPE:
                *value = Subcatch[index].slope; break; // the unit of slope change to 1 not % which is used in swmm. Xichao Gao.
            case SM_CURBLEN:
                *value = Subcatch[index].curbLength * UCF(LENGTH); break;
            // add three subcatchment infiltration parameters. Xichao Gao.
            case SM_SUCT:
                *value = Subcatch[index].S * UCF(RAINDEPTH);
                break;
            case SM_CONDUCT:
                *value = Subcatch[index].Ks * UCF(RAINFALL);
                break;
            case SM_IMD:
                *value = Subcatch[index].IMDmax;
                break;
            case SM_RAINMULTI:    // used to export rainfall multiplier. Xichao Gao.
                *value = Subcatch[index].rainmultiplier;
                break;
            case SM_PERV_N:
                *value = Subcatch[index].subArea[PERV].N;
                break;
            case SM_IMPERV_N:
                *value = Subcatch[index].subArea[IMPERV0].N;
                break;
            case SM_PERV_STO:
                *value = Subcatch[index].subArea[PERV].dStore * UCF(RAINDEPTH);
                break;
            case SM_IMPERV_STO:
                *value = Subcatch[index].subArea[IMPERV1].dStore * UCF(RAINDEPTH);
                break;
            default: error_code_index = ERR_API_OUTBOUNDS; break;
        }
    }
...
/// Purpose: Sets Subcatchment Parameter
{
    ...
    else
    {
        switch(Param)
        {
            ...
            case SM_FRACIMPERV:
                Subcatch[index].fracImperv = MIN(value, 1.0);
                subcatch_setSubareaParams(index); // used to set params that determined by fracImperv. Xichao Gao.
                break; 
            case SM_SLOPE:
            ...
            case SM_SUCT:
                Subcatch[index].S = value / UCF(RAINDEPTH);
                break;
            case SM_CONDUCT:
                Subcatch[index].Ks = value / UCF(RAINFALL);
                break;
            case SM_IMD:
                Subcatch[index].IMDmax = value;
                break;
            case SM_RAINMULTI:    // used to set rainfall multiplier. Xichao Gao.
                Subcatch[index].rainmultiplier = value;
                break;
            case SM_PERV_N:
                Subcatch[index].subArea[PERV].N = value;
                break;
            case SM_IMPERV_N:
                Subcatch[index].subArea[IMPERV0].N = value;
                Subcatch[index].subArea[IMPERV1].N = value;
                break;
            case SM_PERV_STO:
                Subcatch[index].subArea[PERV].dStore = value / UCF(RAINDEPTH);
                break;
            case SM_IMPERV_STO:
                Subcatch[index].subArea[IMPERV1].dStore = value / UCF(RAINDEPTH);
                break;
            default: error_code_index = ERR_API_OUTBOUNDS; break;
        }
